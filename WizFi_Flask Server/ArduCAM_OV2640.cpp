...
void ArduCAM:: Arducam_init(SPIClassRP2040 *ps, TwoWire *pw)
{
	/*
	Wire.setSDA(8);
	Wire.setSCL(9);
...
{
	uint8_t value = bus_read(addr & 0x7F);
	return value;
}
...
{
    p_spi->transfer(BURST_FIFO_READ);
}
...
uint8_t ArduCAM::bus_write(uint8_t address, uint8_t value)
{	
	//cbi(P_CS, B_CS);
	CS_LOW();
	p_spi->transfer(address);
...
uint8_t ArduCAM:: bus_read(uint8_t address)
{
	uint8_t value;
	//cbi(P_CS, B_CS);
	CS_LOW();
	p_spi->transfer(address);
  value = p_spi->transfer(0x00);
	//sbi(P_CS, B_CS);
	CS_HIGH();
	return value;
}
...
{
		p_wire->beginTransmission(sensor_addr >> 1);
	  p_wire->write(regID >> 8);            // sends instruction byte, MSB first
	  p_wire->write(regID & 0x00FF);
	  p_wire->write(regDat & 0x00FF);
	  if (p_wire->endTransmission())
	  {
	    return 0;
	  }
	  delay(1);
...
{	
	  ...
	  p_wire->requestFrom((sensor_addr >> 1), 1);
	  if (p_wire->available())
	    *regDat = p_wire->read();
	  delay(1);
  return 1;
...
uint32_t ArduCAM::transferBytes(uint8_t * out, uint8_t * in, uint32_t size)
{
	p_spi->transfer(out, in, (size_t)size);
